...
// A segment that displays text based on a selection of segments.
// Parameters:
...
// Parameters:
//   args: A record type of:
//     'bars': A list of the 'Segment's objects or strings to parse.
//     'last': A node to splice this into as its successor.
function MultiSegment(args) {
  this.bars = [];
  if (args.bars) {
    for (var i = 0; i < args.bars.length; ++i) {
      var bar = args.bars[i];
      if (bar instanceof Segment) {
        this.bars.push(bar);
      } else if (typeof bar == "string") {
        this.bars.push(parseNyanBar(bar));
      } else {
...
MultiSegment.prototype.getFullLength = function(numSteps) {
  var max = 0;
  for (var i = 0; i < this.bars.length; ++i) {
    max = Math.max(max, this.bars[i].getFullLength(numSteps));
  }
  return max;
};
...
MultiSegment.prototype.getSequenceString = function() {
  var strs = [];
  for (var i = 0; i < this.bars.length; ++i) {
    strs.push(this.bars[i].getSequenceString());
  }
  return strs.join("\n");
};
...
MultiSegment.prototype.update = function(generations) {
  for (var i = 0; i < this.bars.length; ++i) {
    this.bars[i].update(generations);
  }
};
...
function progressBar(options) {
  ...
  if (options.div) {
    ...
    var id = "randID" + ("" + Math.random()).substr(2);
  ...
  var nyanBar = null;
  if (options.nyanBar) {
    if (typeof options.nyanBar == "string") {
      try {
        nyanBar = parseNyanBar(options.nyanBar);
      } catch (e) {
        console.log("Error parsing Nyan Bar: '" + options.nyanBar + "'");
      }
    } else {
      nyanBar = options.nyanBar;
    }
  } else {
  ...
  // Fills the div with a display of the nyan bar or indeterminate.
